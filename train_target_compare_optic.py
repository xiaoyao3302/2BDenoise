...
for epoch in range(start_epoch, args.epochs):
    ...
    for data_all in train_loader:
        ...
        if args.loss_function == 'CE':
            ...
            loss_OD = loss_cat_wise[1]
            loss_OC = loss_cat_wise[2]

            if args.use_CSS_sample:
                # account wise occurance
                total_OC = (noisy_mask == 2).sum()
                total_OD = (noisy_mask == 1).sum()
                total_bg = (noisy_mask == 0).sum()

                all_pixel = noisy_mask.shape[-1] * noisy_mask.shape[-2]

                factor_OC = torch.sqrt(args.CSS_factor / (total_OC / all_pixel))
                factor_OD = torch.sqrt(args.CSS_factor / (total_OD / all_pixel))

                loss_OD = loss_OD * factor_OD
                loss_OC = loss_OC * factor_OC

                loss_seg = (loss_bg + loss_OC + loss_OD) / 3

                print_losses['total'] += loss_seg.item() * batch_size
                print_losses['LovaszSoftmax'] += loss_seg.item() * batch_size
                print_losses['OC'] += loss_OC.item() * batch_size
                print_losses['OD'] += loss_OD.item() * batch_size
        ...
        if (batch_idx % (len(train_loader) // 5) == 0):
            if args.loss_function == 'CE':
                ...
                tb.add_scalar('train_loss_OC', print_losses['OC'] / (batch_idx+1), iters)
                tb.add_scalar('train_loss_OD', print_losses['OD'] / (batch_idx+1), iters)
