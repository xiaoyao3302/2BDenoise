# save target data with DA noise
# note to set the image size here

# v2: remove ImageNet Normalized
# generate DA noise and JED noise at the same time
# 只是为了存可视化数据 没什么用
import numpy as np
import random
import torch
...
io.cprint("-----------------------------------------------------------------")
Val_loss_mean, Val_result_mean = evaluate_save_optic('Val', io, device, args.save_path, model, pretrained_val_loader, idx_epoch=0, need_save=False)
io.cprint("-----------------------------------------------------------------")

io.cprint("-----------------------------------------------------------------")
io.cprint("then verify performance on the new val set of %s" % (args.dataset))
io.cprint("-----------------------------------------------------------------")
Val_loss_mean, Val_result_mean = evaluate_save_optic('Val', io, device, args.save_path, model, val_loader, idx_epoch=0, need_save=False)
io.cprint("-----------------------------------------------------------------")
...
with torch.no_grad():
    ...
    for data_all in train_loader:
        ...
        for ii in range(batch_size):
            ...
            # noisy
            # OD
            mask_bg_OD = torch.zeros([1, img_H, img_W])
            mask_bg_OD[mask == 1] = 1
            ...
            # OC
            mask_bg_OC = torch.zeros([1, img_H, img_W])
            mask_bg_OC[mask == 2] = 1
            ...
            mask_ed = torch.zeros([1, img_H, img_W])
            ...
            color_folder = args.save_path + '/train_vis/'
            os.makedirs(color_folder, exist_ok=True)
            color_path = os.path.join(color_folder, image_name + "_DA_noisy.png")
            gray = Image.fromarray(gray)
            color.save(color_path)
