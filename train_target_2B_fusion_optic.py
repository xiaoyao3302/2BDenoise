...
import copy
import utils.log
from evaluate_optic import evaluate_save_optic_v2, save_filter_prediction_vis
...
for epoch in range(start_epoch, args.epochs):
    ...
    for data_all in train_loader:
        ...
        if (batch_idx % (len(train_loader) // 5) == 0):
            tb.add_scalar('same_pred_ratio', same_pred_ratio / (batch_idx+1), iters)

            tb.add_scalar('train_loss_total', print_losses['total'] / (batch_idx+1), iters)
            tb.add_scalar('train_loss_clean_seg', print_losses['clean_seg'] / (batch_idx+1), iters)
            tb.add_scalar('train_loss_noisy_seg', print_losses['noisy_seg'] / (batch_idx+1), iters)
    ...
    # ===================
    io.cprint("-----------------------------------------------------------------")
    val_loss, val_result = evaluate_save_optic_v2('Val', io, device, args.save_path, model, val_loader, epoch, need_save=args.save_vis_epoch)
...
# ===================
io.cprint("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
Val_loss_mean, Val_result_mean = evaluate_save_optic_v2('Val', io, device, args.save_path, best_mean_model, val_loader, idx_epoch=best_epoch_mean, need_save=True)
