...
def create_optic_colormap_confidence():
    colormap = np.zeros((256, 3), dtype=np.uint8)
    colormap[1] = [128, 0, 0]
    colormap[2] = [0, 128, 0]
...
def evaluate_save_optic(mode, io, device, save_path, model, loader, idx_epoch=0, need_save=True, normalized=True):
    # treat both pred = 1 or 2 as disk
    count = 0
    print_losses = {'total': 0.0, 'seg': 0.0}

    print_results = {'mean_IoU': 0.0, 'OD_IoU': 0.0, 'OC_IoU': 0.0, 'mean_Dice': 0.0, 'OD_Dice': 0.0, 'OC_Dice': 0.0}
...
def evaluate_save_optic_v2(mode, io, device, save_path, model, loader, idx_epoch=0, need_save=True, normalized=True):
    # treat both pred = 1 or 2 as disk
    # v2: add colormap_confidence
    count = 0
    print_losses = {'total': 0.0, 'seg': 0.0}
...
def evaluate_save_optic_VolMin(mode, io, device, save_path, model, trans, loader, idx_epoch=0, need_save=True, normalized=True):
    # treat both pred = 1 or 2 as disk
    # reproduce VolMin
    count = 0
    print_losses = {'total': 0.0, 'seg': 0.0}
